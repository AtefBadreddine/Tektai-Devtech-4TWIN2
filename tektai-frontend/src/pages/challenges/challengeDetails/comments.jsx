...
function commentDate(dateString) {
  ...
  return (
    ...
    {Array.isArray(comments) ? (
      comments.map((comment, index) => (
        <div className="flex-1" key={index}>
              ...
              <div className="flex gap-4 items-center"> {/* Moved the profile image wrapper here */}
                {user && user.image ? (
                  <img
                    src={`http://localhost:3000/uploads/${user.image}`}
                    alt="Profile"
                    className="shadow-xl rounded-full h-10 w-10 object-cover align-middle border-none -ml-4" // Adjusted the margin
                  />
                ) : (
                  <img
          ...
          <div class="flex gap-6 text-[#4b587c] text-[12px] mt-4">
            <span class="flex items-center gap-1 cursor-pointer"><span class="text-[8px]"><svg fill="#4b587c" height="15px" width="15px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
    ...
    <span class="flex items-center gap-1 ">{commentDate(comment.date)}</span>
        ...
        <div class="icons-box">
            <div class="icons">
                <label class="btn-label" for="like-checkbox">
                    <span class="like-text-content">{comment.likes}</span>
          ...
          <hr className="my-4 border-gray-300" /> {/* Gray line */}
        </div>
              ))
            ) : (
